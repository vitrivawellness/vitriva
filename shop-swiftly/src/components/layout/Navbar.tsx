...
import HeroLogo from "/Hero.jpeg"; // Ensure the path is correct relative to your public or src folder
    ...
    <header className="sticky top-0 z-50 bg-[#0A61A6] text-white shadow-md h-20 flex items-center">
      <div className="container-wide flex items-center justify-between w-full">
        <Link to="/" className="flex items-center gap-3 group">
          {/* 2. Updated Logo Container */}
          <div className="w-11 h-11 bg-white rounded-xl flex items-center justify-center shadow-soft group-hover:scale-105 transition-transform duration-300 overflow-hidden">
            <img
              src={HeroLogo}
              alt="Vitriva Logo"
        ...
        <nav className="hidden md:flex items-center gap-10 text-sm font-semibold tracking-wide uppercase">
          ...
          <Link to="/products?category=magnesium" className="text-white/80 hover:text-white transition-all">Magnesium</Link>
          <Link to="/products?category=wellness" className="text-white/80 hover:text-white transition-all">Wellness</Link>
        </nav>
      ...
      {/* Mobile Menu */}
      {mobileOpen && (
        <div className="absolute top-20 left-0 w-full md:hidden border-t bg-white text-[#0A61A6] px-4 py-6 space-y-4 shadow-xl">
